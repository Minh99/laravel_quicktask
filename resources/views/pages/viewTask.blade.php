...
<div class="col-sm">
    ...
    <form action="{{route('task.create')}}" method="POST">
        ...
        <div class="form-group mb-2">
            <label class="mb-2 " for="name_task">{{__('Name')." ".__('Task')}}</label>
            <input type="text" class="form-control" id="name_task" name="name" placeholder='{{__("Enter") . " " .__("Name") . " " . __("Task")}}'>
        </div>
        <button type="submit" class="btn btn-dark text-secondary bg-white"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus" viewBox="0 0 16 16">
            ...
            </svg>{{__('Add')." ".__('Task')}}</button>
    </form>
    <table class="table  table-striped">
        <thead class="thead-light">
            <tr>
                <th scope="col">ID {{__('Task')}}</th>
