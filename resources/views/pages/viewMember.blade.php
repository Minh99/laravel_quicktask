...
<div class="col-sm">
    ...
    <h3 class="text-center">{{__('Members')}}</h3>
    @if(session("status"))
    ...
    <form action="{{route('member.create')}}" method="POST">
        ...
        <div class="form-group mb-2">
            <label class="mb-2 font-weight-bold" for="name_member">{{__('Name')." ".__('Member')}}</label>
            <input type="text" class="form-control" id="name_member" name="name" placeholder='{{__("Enter") . " " .__("Name") . " " . __("Member")}}'>
        </div>
        <button type="submit" class="btn btn-dark text-secondary bg-white"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus" viewBox="0 0 16 16">
            ...
            </svg>{{__('Add') . " " . __('Member')}}</button>
    </form>
    <table class="table  table-striped">
        <thead class="thead-light">
            <tr>
                <th scope="col">ID {{__('Member')}}</th>
                <th scope="col">{{__('Name')." ".__('Member')}}</th>
            </tr>
        </thead>
        <tbody>
